  ...
  $.getJSON("new-books.json", function(data) {
    ...
    $.each(data, function(i, book) {
      if (book["thumbnail"] != "" && book["thumbnail"] != "Too Many Requests") {
        var slide = `
        <a class="card" target="_top" aria-label="` + book["title"] + ` / ` + book["creator"] + `" href="https://epfl.swisscovery.slsp.ch/discovery/fulldisplay?docid=alma` + book["id"] + `&vid=41SLSP_EPF:prod" title="` + book["title"] + `">
